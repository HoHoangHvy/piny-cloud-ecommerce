<template>
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <div class="w-full md:w-1/2">
                        <form class="flex items-center">
                            ...
                            <div class="relative w-full">
                                ...
                                <input type="text" id="simple-search" placeholder="Search for Customers" required="" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            </div>
                        </form>
                    </div>
                    <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <button v-show="store.getters.hasPermission({'module': moduleName, 'action': 'create'})" type="button" id="createCustomerButton" data-modal-target="create-Customer-modal" data-modal-toggle="create-Customer-modal" class="flex items-center justify-center text-white bg-primary hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary focus:outline-none dark:focus:ring-primary-800">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tr v-if="store.getters['Customers/isLoading']">
                            <td colspan="8" class="p-4 text-center">Loading...</td>
                        </tr>
                        <tr v-if="!store.getters['Customers/isLoading'] && list.length === 0">
    ...
    <!-- End block -->
    <div id="create-Customer-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-999 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] md:h-full">
    ...
    <form action="#" id="drawer-update-customer" class="fixed top-0 left-0 z-999 w-full h-screen max-w-3xl p-4 overflow-y-auto transition-transform -translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-update-customer-label" aria-hidden="true">
        <h5 id="drawer-label" class="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase dark:text-gray-400">Edit Customer</h5>
...
// Fetch Customers data from Vuex store
...
const handleCreateCustomer = async () => {
    try {
        await store.dispatch('customers/createCustomer', { ...form });
        notify({
            group: "foo",
            title: "Success",
...
const handleUpdateCustomer = async () => {
    try {
        await store.dispatch('Customers/updateCustomer', {
            id: formEdit.id,
...
const handleDeleteCustomer = async (id) => {
    try {
        await store.dispatch('Customers/deleteCustomer', id);
        notify({
            group: "foo",
            title: "Success",
