        ...
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
            <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    <div v-if="step === 1">
                        <label for="phone" class="block mb-2 font-inter text-black dark:text-white">{{ t('LBL_PHONE_NUMBER') }}</label>
                        <input type="text"
                               id="phone"
                               v-model="phone"
                               placeholder=""
                               ...
                               required="" />
                        <button @click="sendCode" class="w-full text-white bg-[#4D2F19] hover:bg-[#4D2F19] font-inter rounded-lg  px-5 py-2.5 mt-4">{{ t('LBL_SEND_CODE') }}</button>
                    </div>

                    <div v-else-if="step === 2">
                        <label for="code" class="block mb-2 font-inter text-black dark:text-white">{{ t('LBL_ENTER_CODE') }}</label>
                        <input type="text" id="code" v-model="code" placeholder="" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required />
                        <button @click="verifyCode" class="w-full text-white bg-[#4D2F19] hover:bg-[#4D2F19]  font-inter rounded-lg text-sm px-5 py-2.5 mt-4">{{ t('LBL_VERIFY_CODE') }}</button>
                    </div>

                    <div class="space-y-4 md:space-y-6" v-else-if="step === 3">
                        <!-- Mật khẩu mới -->
                        <div class="input-container">
                            <label for="new-password" class="block mb-2 text-sm font-inter text-black dark:text-white">{{ t('LBL_NEW_PASSWORD') }}</label>
                            <div class="flex items-center relative">
                                ...
                                <span @click="toggleNewPassword" class="input-icon cursor-pointer">
                                    <svg v-if="!showNewPassword" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-6 h-6 text-gray-700 dark:text-gray-300">
                        ...
                        <!-- Xác nhận mật khẩu -->
                        <div class="input-container">
                            <label for="confirm-password" class="block mb-2 text-sm font-inter text-black dark:text-white">{{ t('LBL_CONFIRM_PASSWORD') }}</label>
                            <div class="flex items-center relative">
                                <input
                        ...
                        <button @click="resetPassword" class="w-full text-white bg-[#4D2F19] hover:bg-[#4D2F19] font-inter rounded-lg text-sm px-5 py-2.5 mt-4">{{ t('LBL_CONFIRM_RESET') }}</button>
                    </div>

                    <!--Popup đổi mật khẩu thành công-->
                    <div v-if="showPopup" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50" @click="closePopup">
                        <div class="bg-white p-8 rounded-lg w-75" @click.stop>
                            <p class="font-inter text-black text-center">{{ t('LBL_UPDATE_SUCCESSFUL') }}!</p>
                            <button @click="closePopup" class="w-full text-white bg-[#4D2F19] hover:bg-[#4D2F19] font-inter rounded-lg text-sm px-5 py-2.5 mt-4">{{ t('LBL_OK') }}</button>
                        </div>
                    </div>
